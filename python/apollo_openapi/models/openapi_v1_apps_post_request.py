...
class OpenapiV1AppsPostRequest(BaseModel):
    """
    
    """ # noqa: E501
    app: OpenAppDTO
    assign_app_role_to_self: StrictBool = Field(description="是否将应用角色分配给当前用户。当设置为true时，当前token用户将获得该应用的完全操作权限", alias="assignAppRoleToSelf")
    admins: List[StrictStr] = Field(description="应用管理员用户名列表。应用负责人默认具有项目管理员权限。管理员可以创建命名空间、集群并分配用户权限")
