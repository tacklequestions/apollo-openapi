...
public class OpenPageDTOOpenInstanceDTO {
  ...
  private List<OpenInstanceDTO> content;
  ...
  **/
  ...
  public OpenPageDTOOpenInstanceDTO content(List<OpenInstanceDTO> content) {
  ...
  **/
  ...
  public void setContent(List<OpenInstanceDTO> content) {
    this.content = content;
  }
  ...
  * @throws IOException if the JSON Object is invalid with respect to OpenPageDTOOpenInstanceDTO
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!OpenPageDTOOpenInstanceDTO.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!OpenPageDTOOpenInstanceDTO.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `OpenPageDTOOpenInstanceDTO` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      if (jsonObj.get("content") != null && !jsonObj.get("content").isJsonNull()) {
        ...
        if (jsonArraycontent != null) {
          ...
          for (int i = 0; i < jsonArraycontent.size(); i++) {
            OpenInstanceDTO.validateJsonObject(jsonArraycontent.get(i).getAsJsonObject());
          };
        }
      }
  }
